<script lang="ts">
    ...
    let { filterTitle, filterList, filterOpen = false } : { filterTitle: string, filterList: Filter[], filterOpen?: boolean }= $props();
...
<fieldset class="filter-group" class:open={dropdownOpen === true}>
    <legend class="filter-legend">
        <button class="dropdown-button" onclick={() => dropdownOpen = !dropdownOpen}>
            {filterTitle}
            <svg class="dropdown-arrow" xmlns="http://www.w3.org/2000/svg" width="16" height="9" viewBox="0 0 16 9" fill="none">
